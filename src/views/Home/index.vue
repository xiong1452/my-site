<template>
  <div
    ...
    v-loading="imgLoadComplete"
  >
    <ul
      class="Swiper-list"
      :style="{ marginTop: moveMarginTop }"
      @transitionend="handletransition($event)"
    >
      <li v-for="(item, n) in banner" :key="n" class="Swiper-Item">
        <Swiper
          @loading="changeLoadImgComplete($event)"
          v-if="slowLoader(n)"
          :src="item.bigImg"
    ...
    <div
      class="Swiper-Icon Swiper-Icon-Up"
      v-show="index > 1"
      @click="changeSwiperIndex(index - 1)"
    >
      <Icon :type="'arrowUp'"></Icon>
    </div>
    <div
      class="Swiper-Icon Swiper-Icon-Down"
      v-show="index < banner.length"
      @click="changeSwiperIndex(index + 1)"
    >
